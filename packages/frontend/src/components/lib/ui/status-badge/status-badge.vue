...
const VARIANT_CLASSES: Record<Variant, { pill: string; dot: string }> = {
  ...
  <span
    :class="
      cn(
        ...
        VARIANT_CLASSES[variant].pill,
      )
    "
  >
    <span :class="cn('size-1.5 rounded-full', VARIANT_CLASSES[variant].dot)" aria-hidden="true" />
    <slot />
  </span>
</template>
