...
/**
 * Wallet review step — CSV-style summary before importing. Shows stat cards
    ...
    <template v-else>
      ...
      <div class="@container/stat-row">
        <div class="grid grid-cols-2 gap-3 @sm/stat-row:grid-cols-3 @lg/stat-row:grid-cols-5">
          <StatCard
            :label="$t('pages.importExport.walletImport.review.transactions')"
            :value="transactionsCount"
            variant="neutral"
          />
          <StatCard
            :label="$t('pages.importExport.walletImport.review.transfersDetected')"
            :value="transfersCount"
            variant="neutral"
          />
          <StatCard
            :label="$t('pages.importExport.walletImport.review.outOfWallet')"
            :value="outOfWalletCount"
            variant="neutral"
          />
          <StatCard
            :label="$t('pages.importExport.walletImport.review.duplicatesSkipped')"
            :value="duplicatesSkippedCount"
            variant="warning"
          />
          <StatCard
            :label="$t('pages.importExport.walletImport.review.willImport')"
      ...
      <Callout
        ...
        variant="warning"
        :title="
          $t('pages.importExport.walletImport.review.warningsTitle', { count: store.parsedResult.warnings.length })
        "
      >
        <ul class="mt-1 list-disc space-y-0.5 pl-5 text-xs">
          <li v-for="(warning, index) in store.parsedResult.warnings" :key="index">
            <span v-if="warning.rowIndex" class="text-muted-foreground">
              {{ $t('pages.importExport.walletImport.review.rowPrefix', { rowIndex: warning.rowIndex }) }}
            </span>
            {{ warning.message }}
          </li>
        </ul>
      ...
      <!-- Duplicates table -->
      <section v-if="store.duplicates.length > 0" aria-labelledby="wallet-duplicates-heading">
        <h3 id="wallet-duplicates-heading" class="text-warning-text mb-1 text-sm font-semibold">
          {{ $t('pages.importExport.walletImport.review.duplicatesTitle', { count: store.duplicates.length }) }}
        </h3>
        <p class="text-muted-foreground mb-3 text-xs">
          {{ $t('pages.importExport.walletImport.review.duplicatesHint') }}
        </p>
      ...
      <div class="flex items-center justify-between gap-3 pt-2">
        <UiButton variant="ghost" @click="store.goBack()">
          ...
          {{ $t('pages.importExport.walletImport.review.back') }}
        </UiButton>

        <UiButton :disabled="isExecuting" @click="handleImport">
          <template v-if="isExecuting">
            <LoaderCircleIcon class="mr-1.5 size-4 animate-spin" />
            {{ $t('pages.importExport.walletImport.review.importing') }}
          </template>
          <template v-else>
            {{ $t('pages.importExport.walletImport.review.importButton', { count: willImportCount }) }}
