...
/**
 * Wallet resolve step — reconciles every Wallet account + non-transfer category
...
const accountQuickActions = computed<QuickAction[]>(() => [
  {
    icon: LinkIcon,
    label: t('pages.importExport.walletImport.resolve.quickActions.mapExactMatches'),
    tooltip: t('pages.importExport.walletImport.resolve.quickActions.tooltips.mapExactMatchesAccounts'),
    onClick: () => store.quickMapExactMatches({ entity: 'accounts' }),
  },
  {
    icon: PlusIcon,
    label: t('pages.importExport.walletImport.resolve.quickActions.createNewForUnmatched'),
    tooltip: t('pages.importExport.walletImport.resolve.quickActions.tooltips.createNewForUnmatched'),
    onClick: () => store.quickCreateNewForUnmatched({ entity: 'accounts' }),
  },
  {
    icon: RefreshCwIcon,
    label: t('pages.importExport.walletImport.resolve.quickActions.reset'),
    tooltip: t('pages.importExport.walletImport.resolve.quickActions.tooltips.reset'),
...
const categoryQuickActions = computed<QuickAction[]>(() => [
  {
    icon: LinkIcon,
    label: t('pages.importExport.walletImport.resolve.quickActions.mapExactMatches'),
    tooltip: t('pages.importExport.walletImport.resolve.quickActions.tooltips.mapExactMatchesCategories'),
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <AccountMappingTable
      ...
      :title="$t('pages.importExport.walletImport.resolve.accounts.sectionTitle')"
      :resolved-label="$t('pages.importExport.walletImport.resolve.resolvedCounterWord')"
      ...
      <template #create-new-cell="{ item }">
        <div class="flex flex-col gap-1">
          <InputField
            ...
            :label="$t('pages.importExport.walletImport.resolve.accounts.currentBalanceLabel')"
            :placeholder="$t('pages.importExport.walletImport.resolve.accounts.currentBalancePlaceholder')"
            @update:model-value="(value) => onBalanceInput({ name: item.name, value })"
          />
          <span class="text-muted-foreground text-xs">
            {{
              $t('pages.importExport.walletImport.resolve.accounts.willCreateHint', {
    ...
    <CategoryMappingTable
      ...
      :title="$t('pages.importExport.walletImport.resolve.categories.sectionTitle')"
      :resolved-label="$t('pages.importExport.walletImport.resolve.resolvedCounterWord')"
    ...
    <div class="flex items-center justify-between pt-2">
      <UiButton variant="ghost" @click="store.goBack()">
        ...
        {{ $t('pages.importExport.walletImport.resolve.footer.back') }}
      </UiButton>
      ...
      >
        {{ $t('pages.importExport.walletImport.resolve.footer.continue') }}
