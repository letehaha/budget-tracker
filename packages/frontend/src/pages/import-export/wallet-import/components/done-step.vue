<template>
  <div v-if="summary" class="@container/wallet-done space-y-6">
    ...
    <Callout
      ...
      variant="success"
      :title="$t('pages.importExport.walletImport.done.successTitle')"
    >
      <p class="text-sm opacity-80">{{ $t('pages.importExport.walletImport.done.successDescription') }}</p>
    </Callout>
    <Callout
      v-else-if="importOutcome === 'partial'"
      variant="warning"
      :title="$t('pages.importExport.walletImport.done.partialTitle', { count: summary.errors.length })"
    >
      <p class="text-sm opacity-80">{{ $t('pages.importExport.walletImport.done.partialDescription') }}</p>
    </Callout>
    <Callout v-else variant="warning" :title="$t('pages.importExport.walletImport.done.allFailedTitle')">
      <p class="text-sm opacity-80">{{ $t('pages.importExport.walletImport.done.allFailedDescription') }}</p>
    </Callout>

    <div class="grid grid-cols-2 gap-3 @sm/wallet-done:grid-cols-3 @lg/wallet-done:grid-cols-4">
      <StatCard :label="$t('pages.importExport.walletImport.done.accountsCreated')" :value="summary.accountsCreated" />
      <StatCard :label="$t('pages.importExport.walletImport.done.accountsLinked')" :value="summary.accountsLinked" />
      <StatCard
        :label="$t('pages.importExport.walletImport.done.categoriesCreated')"
        :value="summary.categoriesCreated"
      />
      <StatCard :label="$t('pages.importExport.walletImport.done.tagsCreated')" :value="summary.tagsCreated" />
      <StatCard
        :label="$t('pages.importExport.walletImport.done.transactionsImported')"
        :value="summary.transactionsImported"
      />
      <StatCard
        :label="$t('pages.importExport.walletImport.done.transfersImported')"
        :value="summary.transfersImported"
      />
      <StatCard
        v-if="summary.outOfWalletImported > 0"
        :label="$t('pages.importExport.walletImport.done.outOfWalletImported')"
        :value="summary.outOfWalletImported"
      />
      <StatCard
        v-if="summary.duplicatesSkipped > 0"
        :label="$t('pages.importExport.walletImport.done.duplicatesSkipped')"
    ...
    <Callout
      ...
      variant="destructive"
      role="alert"
      :title="$t('pages.importExport.walletImport.done.balanceWarningTitle')"
    >
      <p class="text-sm opacity-80">{{ $t('pages.importExport.walletImport.done.balanceWarningBody') }}</p>
    ...
    <Callout v-if="summary.errors.length > 0" variant="warning">
      <p class="text-sm font-medium">
        {{ $t('pages.importExport.walletImport.done.errorsTitle', { count: summary.errors.length }) }}
      </p>
        ...
        <li v-for="(e, i) in summary.errors.slice(0, MAX_ERRORS_SHOWN)" :key="i">
          ...
          <template v-if="e.rowIndex != null">
            <span class="text-muted-foreground">
              {{ $t('pages.importExport.walletImport.preview.rowPrefix', { rowIndex: e.rowIndex }) }}
            </span>
          </template>
          {{ e.error }}
        </li>
      ...
      <p v-if="summary.errors.length > MAX_ERRORS_SHOWN" class="text-muted-foreground mt-1 text-xs">
        {{
          $t('pages.importExport.walletImport.done.errorsOverflow', { count: summary.errors.length - MAX_ERRORS_SHOWN })
        }}
      </p>
    </Callout>
    ...
    <div class="flex flex-wrap items-center gap-3">
      <UiButton @click="goToTransactions">
        {{ $t('pages.importExport.walletImport.done.viewTransactions') }}
      </UiButton>
      <UiButton variant="ghost" @click="store.reset()">
        {{ $t('pages.importExport.walletImport.done.importAnother') }}
      </UiButton>
    </div>
  </div>
</template>
