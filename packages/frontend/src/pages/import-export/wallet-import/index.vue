  ...
  <div class="@container/csv-wizard flex flex-col gap-0">
    ...
    <div class="mb-6">
      ...
      <h2 class="mb-2 text-2xl font-semibold text-balance">
        {{ $t('pages.importExport.walletImport.pageTitle') }}
      </h2>
      <p class="text-muted-foreground text-sm">
        {{ $t('pages.importExport.walletImport.pageDescription') }}
      </p>
    </div>
...
import { type WalletImportStepKey, useImportWalletStore } from '@/stores/import-wallet';
...
const STEP_LABEL_KEYS: Record<WalletImportStepKey, string> = {
  upload: 'pages.importExport.walletImport.stepper.steps.upload',
  resolve: 'pages.importExport.walletImport.stepper.steps.resolve',
  review: 'pages.importExport.walletImport.stepper.steps.review',
  results: 'pages.importExport.walletImport.stepper.steps.results',
};
...
/** The stepper only emits keys for reachable (completed) steps, all valid WalletImportStepKeys. */
function onNavigate(key: string) {
  store.goToStep(key as WalletImportStepKey);
}

onMounted(() => {
  store.reset();
  trackAnalyticsEvent({ event: 'import_opened', properties: { import_type: 'wallet' } });
});
</script>
