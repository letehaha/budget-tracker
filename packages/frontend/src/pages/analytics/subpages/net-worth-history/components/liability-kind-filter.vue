<template>
  <div v-if="kinds.length > 0" class="w-[190px] max-w-full">
    <MultiSelectField
      v-model:open="isOpen"
      :active="isNarrowed"
      :label="$t('netWorthHistory.kindFilter.all')"
    ...
    >
      <div class="border-border/60 text-muted-foreground border-b px-3 py-2 text-xs">
        {{ $t('netWorthHistory.kindFilter.scopeHint') }}
      </div>
      ...
      <div class="p-2">
        ...
        >
          ...
          <span class="min-w-0 flex-1 truncate text-sm">{{ $t(ACCOUNT_CATEGORIES_TRANSLATION_KEYS[kind]) }}</span>
        </div>
      </div>
    </MultiSelectField>
  </div>
...
/**
 * Liability-kind filter for the Net Worth History report. Purely client-side:
 ...
 * (`kinds`), so the filter disappears entirely for debt-free ranges.
 */
const props = withDefaults(
  defineProps<{
    modelValue?: endpointsTypes.NetWorthLiabilityKind[];
    // Kinds with a nonzero balance anywhere in the loaded series, in display order.
    kinds: endpointsTypes.NetWorthLiabilityKind[];
  }>(),
  {
    modelValue: () => [],
  },
...
const emit = defineEmits<{
  'update:modelValue': [value: endpointsTypes.NetWorthLiabilityKind[]];
}>();
...
const includedKinds = computed<Set<endpointsTypes.NetWorthLiabilityKind>>(
...
const isChecked = (kind: endpointsTypes.NetWorthLiabilityKind) => includedKinds.value.has(kind);

const selectedLabel = computed(() =>
  selectedCount.value === 1
    ? t('netWorthHistory.kindFilter.selectedOne')
    : t('netWorthHistory.kindFilter.selectedMany', { n: selectedCount.value }),
);

const toggleKind = (kind: endpointsTypes.NetWorthLiabilityKind) => {
