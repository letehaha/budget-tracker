...
export const WALLET_MAX_ROWS = 100_000;
...
export const WALLET_TRANSFER_CATEGORY = 'Transfer, withdraw';
...
export const WALLET_CSV_DELIMITER = ';';
...
export interface WalletParseAccount {
 ...
 *  category (`WALLET_TRANSFER_CATEGORY`) is excluded — it is never created. */
export interface WalletParseCategory {
  name: string;
  transactionCount: number;
}
...
export interface WalletParseTag {
  name: string;
  transactionCount: number;
}
...
export interface WalletParseTransaction {
  rowIndex: number;
...
export interface WalletParseTransfer {
...
export interface WalletParseWarning {
  rowIndex: number;
  code:
    | 'unparseable-amount'
    | 'unparseable-date'
...
export interface WalletParseResult {
  accounts: WalletParseAccount[];
  categories: WalletParseCategory[];
  tags: WalletParseTag[];
  transactions: WalletParseTransaction[];
  transfers: WalletParseTransfer[];
  warnings: WalletParseWarning[];
 ...
 *    in the UI.
 */
export type WalletAccountMappingValue =
  | { action: 'create-new'; currencyCode: string; currentBalance: number | null }
  | { action: 'link-existing'; accountId: string };

/** Keyed by `WalletParseAccount.originalName`. */
export type WalletAccountMapping = Record<string, WalletAccountMappingValue>;

// ---------------------------------------------------------------------------
// Duplicate detection
// ---------------------------------------------------------------------------

export interface DetectWalletDuplicatesRequest {
  fileContent: string;
...
export interface DetectWalletDuplicatesResponse {
...
export interface ExecuteWalletRequest {
  fileContent: string;
  accountMapping: WalletAccountMapping;
  /** Per-category decision keyed by the verbatim Wallet `category` value
   *  (matching `WalletParseCategory.name`). Each distinct non-transfer category
   *  is mapped to an existing category (`link-existing`) or created fresh
   *  (`create-new`). The transfer-marker category (`WALLET_TRANSFER_CATEGORY`)
...
export interface ExecuteWalletResponse {
  jobId: string;
}
...
/** Wallet import jobs use the shared import-job lifecycle. Re-exported under the
 *  Wallet-specific names so existing consumers keep working. */
export const WALLET_IMPORT_JOB_STATUSES = IMPORT_JOB_STATUSES;
export type WalletImportJobStatus = ImportJobStatus;

/** Machine-recognizable Wallet-import failure codes the UI special-cases.
...
export type WalletImportErrorCode = 'account-balance-desync';
...
export interface WalletImportSummary {
  ...
  errors: { rowIndex: number | null; error: string; code?: WalletImportErrorCode }[];
}

/** Common counters every progress event carries. */
interface WalletImportProgressBase {
  jobId: string;
...
export type WalletImportProgress =
  | (WalletImportProgressBase & { status: 'queued' | 'running' })
  | (WalletImportProgressBase & { status: 'completed'; summary: WalletImportSummary })
  | (WalletImportProgressBase & { status: 'failed'; error: string });
