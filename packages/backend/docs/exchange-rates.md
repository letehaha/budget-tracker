...
2. **Frankfurter** — secondary. Free, self-hosted, ECB-based. ~31 currencies, overlaps heavily with #1. Also has a time-series endpoint, also goes back to 1999-01-04.
3. **ApiLayer (Fixer)** — tertiary. Paid, external, 150+ currencies. This is the one that covers the long tail — all those exotic currencies the ECB-based providers simply don't publish.
...
flowchart TD
    ...
    HistReg --> P2H[Frankfurter]
    HistReg -.skip.-> P3H[ApiLayer]

    DayReg --> P1[Currency Rates API]
    DayReg --> P2[Frankfurter]
    DayReg --> P3[ApiLayer — fills tail]

    P1 & P2 & P3 --> Merge[mergeProviderRates<br/>gap-fill by priority]
    P1H & P2H --> Merge
...
The range is `1999-01-04` (earliest ECB data) through today, processed in **yearly chunks** to keep memory under control on the bulk insert. Each chunk goes through `fetchHistoricalRatesWithFallback`, which only queries providers that opt in via `supportsHistoricalDataLoading=true` — that's Currency Rates API + Frankfurter. ApiLayer is intentionally skipped here, more on that below.
...
**No time-series endpoint here.** Currency Rates API and Frankfurter both expose a proper `start..end` range endpoint — a whole year of history is a single HTTP call. ApiLayer's Fixer endpoint we use is single-date only (`/fixer/{date}`). So historical via ApiLayer would mean one quota hit per day — strictly worse than the alternatives that already cover the same mainstream currencies for free.
...
`currency-rates-api` (image `letehaha/currency-rates-api`) and `frankfurter` (image `lineofflight/frankfurter`) are both self-hosted in docker-compose. ApiLayer is the only external paid dep — and as covered above, it's only used where it actually earns its keep.
