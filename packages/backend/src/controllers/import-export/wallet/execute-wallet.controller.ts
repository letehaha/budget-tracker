...
import { queueWalletImport } from '@services/import-export/wallet-import';
import { z } from 'zod';

import { categoryMappingValueSchema } from '../shared-schemas';
import { walletAccountMappingSchema } from './shared-schemas';

export const executeWalletController = createController(
  z.object({
    body: z.object({
  ...
  async ({ user, body }) => {
    const { fileContent, accountMapping, categoryMapping, skipDuplicateIndices } = body;
    const jobId = await queueWalletImport({
      userId: user.id,
      fileContent,
      accountMapping,
      categoryMapping,
