...
export const ZodSettingsSchema = z.object({
  ...
  // When true, Payee auto-extraction (Type A + Type B) falls back to the
  // transaction description/note if the provider's dedicated merchant field
  // is empty. Off by default — Monobank's `counterName` is empty for most
  // card purchases, so users have to opt in to use `description` instead.
