...
/**
 * Merge-on-import: an imported bank row confirming a planned transaction the user entered
 * up front. Driven through the CSV wizard endpoints exactly as the UI drives them.
 */
describe('Planned transactions – merge on import', () => {
  ...
  describe('CSV merge happy path', () => {
    it('folds the bank row into the plan and keeps every field the user entered', async () => {
      ...
      const [payee, planTag] = await Promise.all([
        ...
        helpers.createTag({ payload: helpers.buildTagPayload({ name: 'plan-tag' }), raw: true }),
      ]);
      ...
      const summary = (
        await runImport({
          ...
          accountMapping: { [CSV_ACCOUNT]: { action: 'link-existing', accountId: account.id } },
      ...
      const withTags = await helpers.getTransactions({ accountIds: [account.id], includeTags: true, raw: true });
  ...
  describe('rows that must not merge', () => {
    it('creates a new row when the amount differs by a cent', async () => {
      const account = await createOwnedAccount({ initialBalance: 1000 });
      const [plan] = await helpers.createPlannedTransaction({
        payload: { accountId: account.id, amount: 250, time: daysFromRow(1), note: 'Plan' },
        raw: true,
      });
    ...
    it('creates a new row when the plan sits outside the seven-day window', async () => {
      const account = await createOwnedAccount({ initialBalance: 1000 });
      const [plan] = await helpers.createPlannedTransaction({
        payload: { accountId: account.id, amount: 250, time: daysFromRow(8), note: 'Plan' },
        raw: true,
      });
  ...
  describe('picking between candidates', () => {
    it('merges into the plan closest in time and leaves the other one planned', async () => {
      ...
      const [farPlan] = await helpers.createPlannedTransaction({
        payload: { accountId: account.id, amount: 250, time: daysFromRow(5), note: 'Far plan' },
        raw: true,
      });
      const [nearPlan] = await helpers.createPlannedTransaction({
        payload: { accountId: account.id, amount: 250, time: daysFromRow(1), note: 'Near plan' },
        raw: true,
      });
  ...
  describe('duplicate detection', () => {
    it('never flags a planned row as a duplicate of the incoming row', async () => {
      ...
      await helpers.createPlannedTransaction({
        payload: { accountId: account.id, amount: 250, time: `${ROW_DAY}T00:00:00.000Z`, note: 'Plan' },
        raw: true,
      });
  ...
  describe('balance recalculation boundary', () => {
    it('lands the same balance whether or not a future-dated plan sits on the account', async () => {
      ...
      await helpers.createPlannedTransaction({
        payload: { accountId: planned.id, amount: 777, time: daysFromRow(45), note: 'Future plan' },
        raw: true,
      });
      ...
      const summary = (
        await runImport({
          ...
          accountMapping: {
            'Planned Account': { action: 'link-existing', accountId: planned.id },
            'Control Account': { action: 'link-existing', accountId: control.id },
          },
          recalculateBalance: true,
        })
      ).summary;
  ...
  describe('backup restore', () => {
    it('restores an archive written before the planned column existed', async () => {
      ...
      await helpers.createTransaction({
        payload: helpers.buildTransactionPayload({ accountId: account.id, amount: 120, note: 'Pre-feature row' }),
        raw: true,
      });
