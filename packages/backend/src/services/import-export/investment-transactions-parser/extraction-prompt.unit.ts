...
AAPL,Apple Inc.,2024-02-12,B,10,182.5,0.99,USD,stocks,99`;
    const { rows, droppedRowCount } = parseAIResponse({ response: csv });
...
WAT,Watney,2024-02-14,B,1,1,0,USD,nonsense,99`;
  ...
  it('counts dropped rows and surfaces them via droppedRowCount', () => {
    const csv = `BTC,Bitcoin,2024-13-99,B,0.05,42000,0,USDT,crypto,90
ETH,Ethereum,2024-01-20,S,1.2,2300,0,USDT,crypto,90
TOOFEW,Cols
BAD,SideOnly,2024-02-01,X,1,1,0,USD,stocks,90`;
    const { rows, droppedRowCount } = parseAIResponse({ response: csv });
    expect(rows.map((r) => r.symbol)).toEqual(['ETH']);
    expect(droppedRowCount).toBe(3);
  });

  it('skips rows with non-positive quantity and counts them as dropped', () => {
...
SOL,Solana,2024-01-21,B,1,95.5,0,USD,crypto,90`;
  ...
  it('returns empty result for empty input', () => {
    expect(parseAIResponse({ response: '' })).toEqual({ rows: [], droppedRowCount: 0 });
    expect(parseAIResponse({ response: '   \n  ' })).toEqual({ rows: [], droppedRowCount: 0 });
  });
