...
interface CreateImportJobQueueParams<TJobData extends SentryTraceData, TSummary, TProgress extends SSEEventPayload> {
  /** Stable queue base name (e.g. `wallet-import`). Namespaced by Jest worker id
  ...
  /** Human label used in log lines (e.g. `Wallet Import`). */
  logLabel: string;
...
interface ImportJobQueueBundle<TJobData extends SentryTraceData, TSummary, TProgress extends SSEEventPayload> {
  ...
  /** Enqueue a job and emit the initial `queued` SSE event. */
  enqueue: (params: { userId: number; jobId: string; data: TJobData; queuedPayload: TProgress }) => Promise<void>;
