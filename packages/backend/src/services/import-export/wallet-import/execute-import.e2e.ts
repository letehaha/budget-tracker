...
import { expectCompleted, waitForWalletCompletion } from '@tests/helpers/import-export';
...
async function buildCreateNewMappingFromFixture({ fileContent }: { fileContent: string }) {
  const { result } = await helpers.parseWallet({ payload: { fileContent }, raw: true });
...
describe('Execute Wallet import endpoint', () => {
  /**
  ...
  it('imports the multi-currency fixture end-to-end: counts, transfer link, out-of-wallet, datetime', async () => {
    const fileContent = helpers.loadWalletFixture('multi-currency.csv');
    const { accountMapping } = await buildCreateNewMappingFromFixture({ fileContent });

    const { jobId } = await helpers.executeWallet({
      payload: { fileContent, accountMapping, skipDuplicateIndices: [] },
      raw: true,
    });
    expect(jobId).toBeTruthy();

    const progress = await waitForWalletCompletion({ jobId });
  ...
  it('sets the new-account currentBalance to the user-supplied value after import', async () => {
    ...
    const fileContent = helpers.loadWalletFixture('basic.csv');
    const { result } = await helpers.parseWallet({ payload: { fileContent }, raw: true });
  ...
  it('detects and skips duplicates on a second import into a linked account', async () => {
    ...
    const { jobId: firstJobId } = await helpers.executeWallet({
      payload: { fileContent, accountMapping, skipDuplicateIndices: [] },
      raw: true,
    });
    expect(firstJobId).toBeTruthy();
    const firstProgress = await waitForWalletCompletion({ jobId: firstJobId });
    ...
    const { duplicates } = await helpers.detectWalletDuplicates({
    ...
    const { jobId: secondJobId } = await helpers.executeWallet({
      payload: { fileContent, accountMapping, skipDuplicateIndices },
      raw: true,
    });
    expect(secondJobId).toBeTruthy();
    const secondProgress = await waitForWalletCompletion({ jobId: secondJobId });
  ...
  it("refuses to leak another user's job status (cross-user authZ)", async () => {
    ...
    const statusAsOther = await asUser({
      ...
      fn: () => helpers.getWalletImportStatus({ jobId }),
    });
  ...
  it('returns 404 for an unknown job id', async () => {
    const response = await helpers.getWalletImportStatus({ jobId: 'no-such-wallet-job' });
