...
describe('parseWalletDate', () => {
  describe('ISO-8601 format (primary Wallet export format)', () => {
    ...
    it('returns null for a malformed ISO-looking string', () => {
      expect(parseWalletDate({ raw: '2025-13-01T00:00:00.000Z' })).toBeNull();
    });
    ...
    it('returns null for an out-of-range day V8 would silently roll over (30 Feb)', () => {
      ...
      expect(parseWalletDate({ raw: '2025-02-30T10:00:00.000Z' })).toBeNull();
    });

    it('returns null for a day that does not exist in a 30-day month (31 Apr)', () => {
      expect(parseWalletDate({ raw: '2025-04-31T00:00:00Z' })).toBeNull();
    });

    it('returns null for 29 Feb in a non-leap year', () => {
      expect(parseWalletDate({ raw: '2025-02-29T12:00:00Z' })).toBeNull();
    });

    it('accepts 29 Feb in a leap year', () => {
      expect(parseWalletDate({ raw: '2024-02-29T12:00:00Z' })).toBe('2024-02-29T12:00:00.000Z');
    });
    ...
    it('treats a bare ISO without timezone designator as UTC (regression guard: no server-offset shift)', () => {
      ...
      const bare = parseWalletDate({ raw: '2025-12-25T14:30:00' });
      const withZ = parseWalletDate({ raw: '2025-12-25T14:30:00Z' });
      expect(bare).toBe('2025-12-25T14:30:00.000Z');
      expect(bare).toBe(withZ);
    });

    it('passes a Z-suffixed ISO through unchanged', () => {
      expect(parseWalletDate({ raw: '2025-12-25T14:30:00Z' })).toBe('2025-12-25T14:30:00.000Z');
    });

    it('respects an explicit positive UTC offset without double-shifting', () => {
      // +02:00 means 14:30 local = 12:30 UTC.
      expect(parseWalletDate({ raw: '2025-12-25T14:30:00+02:00' })).toBe('2025-12-25T12:30:00.000Z');
    });
    ...
    it('respects a negative UTC offset (-05:00): 02:00 local = 07:00 UTC', () => {
      ...
      expect(parseWalletDate({ raw: '2025-12-25T02:00:00-05:00' })).toBe('2025-12-25T07:00:00.000Z');
    });
    ...
    it('appends Z to a bare ISO string that includes milliseconds but no timezone', () => {
      ...
      expect(parseWalletDate({ raw: '2025-12-25T14:30:00.123' })).toBe('2025-12-25T14:30:00.123Z');
    });

    it('returns null for an invalid ISO string even without a timezone designator', () => {
      expect(parseWalletDate({ raw: '2025-13-01T00:00:00' })).toBeNull();
    });
  });

  describe('DD/MM/YYYY HH:MM format (older Wallet exports)', () => {
    it('parses a valid DD/MM/YYYY HH:MM date', () => {
      expect(parseWalletDate({ raw: '25/12/2025 14:30' })).toBe('2025-12-25T14:30:00.000Z');
    });

    it('parses midnight correctly', () => {
      expect(parseWalletDate({ raw: '01/01/2026 00:00' })).toBe('2026-01-01T00:00:00.000Z');
    });

    it('returns null for an invalid calendar date (30 Feb)', () => {
      expect(parseWalletDate({ raw: '30/02/2025 10:00' })).toBeNull();
    });

    it('returns null for an out-of-range month', () => {
      expect(parseWalletDate({ raw: '01/13/2025 10:00' })).toBeNull();
    });

    it('returns null for an out-of-range hour', () => {
      expect(parseWalletDate({ raw: '01/01/2025 25:00' })).toBeNull();
    });
    ...
    it('returns null for MM/DD/YYYY HH:MM (not the Wallet format — prevents accidental swap)', () => {
      ...
      const result = parseWalletDate({ raw: '31/07/2025 13:30' });
      expect(result).toBe('2025-07-31T13:30:00.000Z');
    });
  });
  ...
  describe('null / empty / garbage', () => {
    it('returns null for empty string', () => {
      expect(parseWalletDate({ raw: '' })).toBeNull();
      expect(parseWalletDate({ raw: '   ' })).toBeNull();
    });

    it('returns null for nullish input', () => {
      expect(parseWalletDate({ raw: null })).toBeNull();
      expect(parseWalletDate({ raw: undefined })).toBeNull();
    });

    it('returns null for unrecognized formats', () => {
      expect(parseWalletDate({ raw: '2025-12-25' })).toBeNull();
      expect(parseWalletDate({ raw: 'Dec 25, 2025' })).toBeNull();
      expect(parseWalletDate({ raw: 'hello' })).toBeNull();
    });
  });
});
