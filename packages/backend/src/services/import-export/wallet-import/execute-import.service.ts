...
import { createCategoriesIfNeeded } from '@services/import-export/csv-import/execute-import/create-categories-if-needed';
...
/**
 * Translate a Wallet `payment_type` cell into the app's PAYMENT_TYPES enum.
 * `Cash` and `Credit card` map to their direct equivalents; every other value
 * (Wallet's `Transfer`, an unrecognized value, a malformed export) falls back to
 * `bankTransfer` so a row never lands without a payment type. The mapping is
 * intentionally lenient — payment type is cosmetic metadata here, not a
 * correctness-critical field.
 */
function mapWalletPaymentType({ raw }: { raw: string }): PAYMENT_TYPES {
  switch (raw.trim()) {
    case 'Cash':
...
interface ExecuteWalletImportParams {
  userId: number;
  fileContent: string;
 ...
 * free).
 */
export async function executeWalletImport({
  userId,
  fileContent,
  accountMapping,
  categoryMapping,
  ...
  onProgress,
}: ExecuteWalletImportParams): Promise<WalletImportSummary> {
  ...
  // Phase 2: accounts. Resolve each parsed account to an app account id and
  // capture a target current balance to restore after all rows are written.
  ...
  for (const account of parsed.accounts) {
    ...
    if (mapping.action === 'create-new') {
      ...
      const created = await Accounts.createAccount({
        ...
        // Start at zero and let the imported rows build the balance up; the
        // user-entered target (if any) is restored in Phase 7.
      ...
      if (!created) {
        throw new ValidationError({ message: `Failed to create account "${account.originalName}".` });
      }
      accountIdByName.set(account.originalName, created.id);
      ...
      accountIdByName.set(account.originalName, existing.id);
  ...
  // Phase 4: tags. Find-or-create by the verbatim label text, batching the
  // existing-lookup just like categories.
  const tagIdByName = new Map<string, string>();
  const tagNames = parsed.tags.map((t) => t.name);
  if (tagNames.length > 0) {
    const existingTags = await Tags.findAll({
      where: { userId, name: { [Op.in]: tagNames } },
    });
  ...
  for (const tag of parsed.tags) {
    ...
    try {
      const created = await createTag({ userId, name: tag.name, color: pickRandomColor() });
      tagIdByName.set(tag.name, created.id);
      summary.tagsCreated += 1;
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unknown error';
      logger.error({ message: `[Wallet import] Failed to create tag "${tag.name}"`, error: err as Error });
      // Tagged transactions will still land — just without this tag. rowIndex is
      // null because the failure spans every row that used this label, not one row.
      summary.errors.push({ rowIndex: null, error: `Failed to create tag "${tag.name}": ${message}` });
    }
  }
  ...
  const tick = async () => {
    ...
    try {
      ...
      logger.error({ message: '[Wallet import] Progress callback failed', error: err as Error });
    }
  };
  ...
  for (const tx of parsed.transactions) {
    ...
    try {
      ...
      await createTransaction({
        ...
        transactionType,
        paymentType: mapWalletPaymentType({ raw: tx.paymentType }),
      ...
      logger.error({
        message: `[Wallet import] Failed to import transaction (row ${tx.rowIndex}, account "${tx.accountName}")`,
        error: err as Error,
      });
      summary.errors.push({
  ...
  for (const xfer of parsed.transfers) {
    try {
      ...
      logger.error({
        message: `[Wallet import] Failed to import transfer ("${xfer.sourceAccountName}" → "${xfer.destinationAccountName}", rows ${xfer.rowIndices.join(', ')})`,
        error: err as Error,
      });
      const message = err instanceof Error ? err.message : 'Unknown error';
  ...
  for (const [accountName, target] of targetCurrentBalanceByName) {
    ...
    try {
      ...
      if (isLinkExisting) {
        ...
        logger.error({
          message: `[Wallet import] Failed to reset balance for linked account "${accountName}" after import — balance may be incorrect`,
          error: err as Error,
        });
        summary.errors.push({
          rowIndex: null,
        ...
        logger.error({
          message: `[Wallet import] Failed to set balance for account "${accountName}"`,
          error: err as Error,
        });
