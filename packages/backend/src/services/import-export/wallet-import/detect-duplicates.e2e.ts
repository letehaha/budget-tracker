...
describe('Detect Wallet Duplicates endpoint', () => {
  describe('POST /import/wallet/detect-duplicates', () => {
    /**
     ...
     *
     * Seeding is done via executeWallet (the only route that writes transactions)
     * to stay strictly HTTP-only — no direct service calls.
     */
    ...
    it('returns 422 for an empty fileContent string', async () => {
      const response = await helpers.detectWalletDuplicates({
        payload: {
          fileContent: '',
          accountMapping: {},
        },
     ...
     *
     * After seeding via executeWallet we call detect-duplicates with the same
     * CSV and a mixed mapping.  Only the two "Linked UAH" rows must be returned.
     */
