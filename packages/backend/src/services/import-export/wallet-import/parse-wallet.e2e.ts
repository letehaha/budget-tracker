 ...
 * is exercised in `parse-wallet.unit.ts`. These tests only smoke-test the HTTP
 * wiring: controller validates the request, delegates to the service, and wraps
 * the result in the expected response envelope.
 */
describe('Parse Wallet endpoint', () => {
  describe('POST /import/wallet/parse', () => {
    ...
    it('surfaces parser validation errors as HTTP 422 for empty file content', async () => {
      ...
      const response = await helpers.parseWallet({ payload: { fileContent: '   ' } });
    ...
    it('returns 422 for a non-empty CSV that is missing required Wallet columns', async () => {
      ...
      const response = await helpers.parseWallet({ payload: { fileContent: malformedCsv } });
      expect(response.statusCode).toBe(ERROR_CODES.ValidationError);
    });
  });
});
