import type { AutomationCondition, RecordId } from '@bt/shared/types';
...
// Deleting a row a rule points at pauses the rule, or rewrites it when the delete named a successor.
...
describe('Transaction automations reference lifecycle', () => {
  ...
  it('refuses to re-enable a broken rule and clears pausedReason once the action is fixed', async () => {
    const token = generateRandomRecordId();
