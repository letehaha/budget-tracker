...
/**
 * Remove the custom configuration for a specific AI feature.
 * The feature will fall back to using the default model.
 */
export const removeFeatureConfig = withTransaction(
  async ({ userId, feature }: { userId: number; feature: AI_FEATURE }): Promise<void> => {
    await setFeatureConfig({ userId, feature, modelId: null });
  },
);

/**
 * Remove all custom feature configurations for a user.
 * All features will fall back to using default models.
 */
export const removeAllFeatureConfigs = withTransaction(async ({ userId }: { userId: number }): Promise<void> => {
  const userSettings = await UserSettings.findOne({
    where: { userId },
  });
