...
const createVehicleImpl = async (params: CreateVehicleParams) => {
  ...
  const vehicle = await Vehicles.create({
    ...
    purchaseDate,
    depreciationPreset,
    // DECIMAL columns are stored as strings; the API surfaces these percentages as numbers.
    customAnnualRatePct: customAnnualRatePct == null ? null : String(customAnnualRatePct),
    salvageFloorPct: salvageFloorPct === undefined ? undefined : String(salvageFloorPct),
    currentMileage,
