...
7. **i18n Files - use the `i18n-editor` subagent**
   ...
   - **All supported locales, always together.** No more Crowdin – every locale is translated in-house. Every new or changed key is written to `en` and translated into every other locale listed in `SUPPORTED_LOCALES` (`packages/shared/src/i18n/locales.ts` – currently `uk`, `es`, `id`) in the same task, so no locale lags behind and needs no separate pre-release pass. Leftover `zh-CN`/`zh-TW` chunk files (from the discontinued Crowdin integration) aren't wired into `SUPPORTED_LOCALES` – leave them alone unless explicitly bringing one into scope.
