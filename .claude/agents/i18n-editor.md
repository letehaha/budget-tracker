...
Sibling locale directories (`uk/`, `es/`, `id/`, `zh-CN/`, …) mirror this tree. They are Crowdin's output – read any of them for context; write only to `uk/` or `es/`, and only during an explicit translation task (see above).
...
The other locale files sit alongside it and belong to Crowdin; edit `uk.json` / `es.json` only during an explicit translation task, and never the rest.
...
1. **`en` by default** - Edit `en/` chunks and backend `en.json`. Other locales come from Crowdin; write to `uk`/`es` only when the task explicitly asks you to translate into them, and never write to any other locale.
