...
Sibling `uk/`, `es/` and `id/` directories mirror this tree. They are Crowdin's output – read them if you need context; write to them only during an explicit translation task (see above).
...
`uk.json`, `es.json` and `id.json` sit alongside it and belong to Crowdin; edit them only during an explicit translation task.
...
1. **`en` by default** - Edit `en/` chunks and backend `en.json`. Other locales come from Crowdin; write to them only when the task explicitly asks you to translate into them.
