...
**Locales:** `en` (English), `uk` (Ukrainian), `es` (Spanish)
...
| Backend | `packages/backend/src/i18n/locales/es.json` |
...
1. **Always update ALL locales** - When editing a chunk, update the same chunk file in every locale (`en/`, `uk/`, `es/` for frontend; `en.json`, `uk.json`, `es.json` for backend). Use English as a placeholder if no translation is provided for a non-English locale.
